...
export const NewsCard: React.FC<NewsCardProps> = ({ haber, isFeatured = false }) => {
  ...
  // Önce gorsel_url, yoksa resim_url kullanılır
  ...
  return (
    ...
    >
      <div>
        {/* Görsel Katmanı (Görsel Varsa Göster, Yoksa Gizle) */}
        {imageUrl ? (
          ...
          /* Görsel Yoksa Sade Üst Bilgi Satırı */
          <div className="p-5 pb-0 flex items-center justify-between">
            <span
              className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded text-[11px] font-bold tracking-wide border uppercase ${getSourceBadgeStyle(
                haber.kaynak_adi
              )}`}
            >
              <Newspaper className="w-3 h-3 text-brass" />
            ...
            <span className="inline-flex items-center gap-1 text-[11px] font-mono text-steel" title={fullDate}>
              <Clock className="w-3 h-3 text-brass" />
              {relativeTime}
            </span>
          </div>
        ...
        <div className="p-5 md:p-6">
          ...
          {/* Haber Özeti (2000 Karakter Destekli Esnek line-clamp) */}
          <p className="font-inter text-ink/80 text-sm md:text-base leading-relaxed line-clamp-5 mb-4 font-normal">
            {haber.ozet}
          </p>
        </div>
      </div>
